...
# perl perl_hmmtop_topology_post_and_save.pl -infile scrap.fasta -rmvggg

# If the -rmvggg flag is present, then remove leading and trailing GGG, HHH and AAA before submitting to prediction tool.

# will produce output file xxx.hmmtop (eg. scrap.fasta.hmmtop)
# will produce output file xxx.hmmtops_html (eg. scrap.fasta.hmmtop_html)
# will produce output file xxx.hmmtop_redo (eg. scrap.fasta.hmmtop_redo)

# The input to this program is a file containing multiple fasta sequences.
# Each fasta sequence is submitted to the HMMTOP server at http://www.enzim.hu/hmmtop/html/submit.html
...
my $url_for_submit = 'http://www.enzim.hu/hmmtop/html/../server/hmmtop.cgi';
...
my $output_file = "$fasta_file.hmmtop";
my $output_file_2 = "$fasta_file.hmmtop_html";
my $redo_file = "$fasta_file.hmmtop_redo";
