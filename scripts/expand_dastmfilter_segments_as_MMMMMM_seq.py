...
# python expand_dastmfilter_segments_as_MMMMMM_seq.py dastmfilter_noblanks_input1.dastmfilter dastmfilter_input1.fasta dastmfilter_noblanks_input1.dastmfilter_HHHHHH

# python expand_dastmfilter_segments_as_MMMMMM_seq.py [input-file-1] [input-file-2] [output-file]
...
def main():
	...
	for inline in inlines1:
		...
		if (inline != ''):
			...
			this_id = bits[0]
			this_info = bits[1]
			bits2 = this_info.rsplit( ';;;;;' )
			this_signalpeptide_info = bits2[0]
			this_segments_info = bits2[1]
			bits3 = this_signalpeptide_info.rsplit( ',,,,,' )
			this_signalpeptide = bits3[1]
			bits4 = this_segments_info.rsplit( ',,,,,' )
			this_segments = bits4[1]
			this_seq_length = 0
			...
			if (this_signalpeptide != '-'):
				bits = this_signalpeptide.rsplit(',')
				for bit in bits:
					bit = bit.strip()
					if (bit != ''):
						bits2 = bit.rsplit('-')
