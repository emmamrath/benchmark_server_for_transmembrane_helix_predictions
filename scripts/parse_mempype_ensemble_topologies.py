...
# python parse_mempype_ensemble_topologies.py alpha_mempype_tCagy082.txt alpha_mempype_tCagy082.noblanks.mempype

# python parse_mempype_ensemble_topologies.py [input-file] [output-file]
...
def main():
	...
	for inline in inlines1:
		...
		if (inline != ''):
			...
			elif ( inline.find("Signal peptide (SPEP):\tYES") > -1 ):
				this_char = 'S'
				bits = inline.split("\t")
				this_from = int(bits[2])
				this_to = int(bits[3])
				this_to = this_to + 1
			...
			elif ( inline.find("Non cytoplasmic region (ENSEMBLE):") > -1 ):
				this_char = 'o'
				bits = inline.split("\t")
				this_from = int(bits[2])
				this_to = int(bits[3])
			...
			elif ( inline.find("Cytoplasmic region (ENSEMBLE):") > -1 ):
				this_char = 'i'
				bits = inline.split("\t")
				this_from = int(bits[2])
				this_to = int(bits[3])
