...
# python list_file1_lower_chain_having_file2_higher_chain_pdbid.py opm_feb2012.beta_barrel.opm_seq opm_feb2012.all_opm_having_pdb_struc2D.pdb_seq possible_missing_chains_in_pdb_struc2D
...
def main():
	...
	for inline in inlines1:
		inline = inline.strip()
		if (inline != ''):
			this_chain = get_chain( inline )
			this_chain_is_lowercase = is_it_lowercase( this_chain )
			if (this_chain_is_lowercase == 1):
				this_pdbid = get_pdbid( inline )
				i = 0
				for this_save_pdbid in save_pdbid:
					if (this_save_pdbid == this_pdbid):
						this_save_id = save_id[i]
						found_it = look_for_output_id( this_save_id )
						if (found_it == 0):
							output_line = this_save_id + "\r\n"
							outfile.write( output_line )
							output_id.append( this_save_id )
					i = i + 1
	outfile.close()
