...
# perl perl_pdbe_taxonomy_post_and_parse_for_id.pl -infile recent.pdb_select25.id
...
foreach my $input_line (@input_lines) {
	...
	if ($input_line ne '') {
		...
		my $id = substr( $input_line, 0, 6 );
		my $pdbid = substr( $input_line, 0, 4 );
		process_1_pdbid( $pdbid, $id );
	}
}
				...
				if ($line =~ /Chain/) {
					...
					if ($in_a_chain == 1) {
						foreach my $this_chain (@current_chains) {
							my $pdbid_chain = $pdbid . '_' . $this_chain;
							if ($pdbid_chain eq $id) {
								my $output_line = $id . ':::::' . $chain_output . ':::::' . "\n";
