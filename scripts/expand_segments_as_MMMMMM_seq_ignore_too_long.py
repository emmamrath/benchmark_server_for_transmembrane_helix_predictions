...
# python expand_segments_as_MMMMMM_seq.py alpha_polytopic_bitopic_noblanks.fasta.split4.adjusted_for_removed_blanks alpha_polytopic_bitopic.split4

# python expand_segments_as_MMMMMM_seq.py [input-file] [output-file]
			...
			if ((this_segments != '-') and (this_segments != '-,')):
				...
				for this_segment in bits2:
					if (this_segment != ''):
						...
						this_from = bits3[0]
						this_to = bits3[1]
						print this_id, this_from, this_to
						this_from = int(this_from)
						this_to = int(this_to)
						if (this_to <= len(this_seq)):
